<?php
  ...
  function rcon_send($command,$sid,$max_response_pages=0) {
    ...
    $resource = mysqli_query($mysql, "SELECT address,rcon FROM ".$config->db_prefix."_serverinfo WHERE id=".$sid) or die (mysqli_error($mysql));
  ...
  if(isset($_POST["save"]) && $_POST["rcon"] != "***PROTECTED***") {
    ...
    $query = mysqli_query($mysql, "UPDATE `".$config->db_prefix."_serverinfo` SET 
  ...
  if(isset($_POST["del"])) {
    ...
    $query = mysqli_query($mysql, "DELETE FROM `".$config->db_prefix."_serverinfo` WHERE `id`=".$sid." LIMIT 1") or die (mysqli_error($mysql));
  ...
  //get reason sets
  $query = mysqli_query($mysql, "SELECT * FROM `".$config->db_prefix."_reasons_set` ORDER BY `setname` ASC") or die (mysqli_error($mysql));

<div class="main">
  <div class="post">
    <table>
      ...
      {/if}
      {if $banedit_error}<br /><div class="_center"><span class="error">{"_ERROR"|lang}: {foreach from=$banedit_error item=banedit_errors}{$banedit_errors|lang}{/foreach}</span></div><br />{/if}
    </table> 
...
{if  $vars.use_demo == 1}
    {if $upload_error}<div class="_center"><span class="error">{"_ERROR"|lang}: {foreach from=$upload_error item=upload_errors}{$upload_errors|lang}{/foreach}</span></div><br />{/if}
      ...
      <tr class="title">
        ...
        {foreach from=$demos item=demo}
          <form method="post">
            ...
            <input type="hidden" name="details_x" value="1" />
            <tr>
              <td>{$demo.upload_time|date_format:"%d.%m.%Y"}</td> 
...
{/if}
  ...
  {if  $vars.use_comment == 1}
    {if $comment_error}<div class="_center"><span class="error">{"_ERROR"|lang}: {foreach from=$comment_error item=comment_errors}{$comment_errors|lang}{/foreach}</span></div><br />{/if}
      ...
      <form method="POST">
        ...
        <input type="hidden" name="details_x" value="1" />
          <table>
              <!-- Comment List -->
              <tr> 
                <td style="width:130px;">{$comment.date|date_format:"%d.%m.%Y"}</td> 
                ...
                <td class="_right" style="width:80px;">
                  {if $smarty.session.bans_edit=="yes" || ($smarty.session.bans_edit=="own" && $smarty.session.uname == $ban_detail.nickname || $smarty.session.uname == $ban_detail.admin_nick)}
                    <img src="images/page_edit.png" border="0" style="cursor:pointer;" onClick="NewToggleLayer('comedit_{$comment.id}');" />

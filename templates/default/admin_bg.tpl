        ...
        <tr>
          <td>
            <table>
              ...
              {foreach from=$reasons_set item=reason_set}
                <form method="POST">
                  <input type="hidden" name="rsid" value="{$reason_set.id}" />
                  <tr class="list">
                    <td>
                      {if $reason_set.setname == ""}&nbsp;{else}{$reason_set.setname}{/if}
                    </td>
                    <td align="center">
                      <input type="button" class="button" onclick="NewToggleLayer('layer_{$reason_set.id}');" value="{"_EDIT"|lang}" />
                      <input type="submit" class="button" name="delset" value="{"_DEL"|lang}" {if $smarty.session.servers_edit !== "yes"}disabled{/if} />
                    </td>
                  </tr>
                  <tr id="layer_{$reason_set.id}" style="display: none">
                    <td colspan="3">
                      ...
                      <table class="details" width="95%">
                        <form method="POST">
                          ...
                          <tr class="info">
                            <td>{"_NAME"|lang}:</td>
                            <td><input type="text" name="setname" value="{$reason_set.setname}" /></td>
                          ...
                          {section name=reasons loop=$reasons}
                            <tr class="info">
                              ...
                              <td><input type="checkbox" name="aktiv[]" value="{$reasons[reasons].id}" {$reason_set.reasonids|strinstr:",":$reasons[reasons].id:"checked"} /></td>
                            </tr>
                          {/section}
                        </form>
                      </table></div>

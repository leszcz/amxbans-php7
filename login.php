...
if(isset($_POST["action"])) {
  ...
  if(!$msg) {
    ...
    $query = mysqli_query($mysql, "SELECT last_action,try FROM `".$config->db_prefix."_webadmins` WHERE username='$uname' LIMIT 1");
    if(mysqli_num_rows($query)) {
      ...
      if($try >= $max_trys) {
        if($last_action+($max_trys_block*60) > time()) {
          ...
          //delete wrong trys
          $query2 = mysqli_query($mysql, "UPDATE `".$config->db_prefix."_webadmins` SET `try`=0 WHERE username='$uname' LIMIT 1");
          $try=0;
        }
      } 
      if(!$loginblocked) {
        //check username and password
        $query = mysqli_query($mysql, "SELECT id,level,email FROM `".$config->db_prefix."_webadmins` WHERE username='$uname' AND password=MD5('$upass') LIMIT 1");
        if(mysqli_num_rows($query)) {
          ...
          $query = mysqli_query($mysql, "SELECT * FROM `".$config->db_prefix."_levels` WHERE level=".$_SESSION["level"]." LIMIT 1");
          ...
          $query = mysqli_query($mysql, "UPDATE `".$config->db_prefix."_webadmins` SET `logcode`='".session_id()."',`last_action`=UNIX_TIMESTAMP(),`try`=0 WHERE `id`=".$_SESSION["uid"]);
          #$msg="_LOGINOK";
          header("Location:index.php");
          exit;
        } else {
